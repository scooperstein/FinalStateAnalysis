...
vetos = PSet(
    ...
    muVetoZH = 'vetoMuons(0.4, "(isGlobalMuon & isTrackerMuon) & pt > 10 & abs(eta) < 2.4 & (userIso(0) + max(photonIso + neutralHadronIso - 0.5*puChargedHadronIso, 0))/pt < 0.3 & userInt(\'looseID\') > 0.5 & userFloat(\'dz\') < 0.1").size()',
    ...
    tauVetoZH = 'vetoTaus(0.1, "pt > 15 & abs(eta) < 2.3 & tauID(\'decayModeFinding\') & tauID(\'againstElectronLoose\') & tauID(\'againstMuonLoose\') & userFloat(\'dz\') < 0.1").size()',
    ...
    eVetoZH = 'vetoElectrons(0.4, "pt > 10 & abs(eta) < 2.5 & (userIso(0) + max(userIso(1) + neutralHadronIso - 0.5*userIso(2), 0))/pt < 0.3 & ( ( abs(eta) < 0.8 & electronID(\'mvaNonTrigV0\') > 0.5 ) || ( abs(eta) > 0.8 & abs(eta) < 1.479 & electronID(\'mvaNonTrigV0\') > 0.12 ) || ( abs(eta) > 1.479 & electronID(\'mvaNonTrigV0\') > 0.6 ))").size()',
